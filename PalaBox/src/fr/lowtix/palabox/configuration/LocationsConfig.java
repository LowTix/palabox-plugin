...
public class LocationsConfig {
	...
	public void saveLocation(Locations path, Location location) {
		...
		FileConfiguration config = Main.getInstance().getConfigurationManager().getLocationsConfig();
		
		config.set(path.name(), serial);
		
		Main.getInstance().getConfigurationManager().saveLocationConfig();
		savedLocs.put(path, location);
	}
	
	public boolean locationExist(Locations path) {
		return Main.getInstance().getConfigurationManager().getLocationsConfig().contains(path.name());
	}
